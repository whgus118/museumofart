...
- **화살표 조준 보정:** 슬라이드 이동 화살표의 가시적 크기는 유지하되, 가상 요소(Pseudo-element)를 활용하여 투명 클릭 영역을 최소 44x44px 이상 확보함으로써 사용자의 조작 오류(오클릭)를 방지하고 인터랙션 성공률을 높임
- **심리적 안도감을 주는 브레드크럼:** 현재 위치(홈 > 체험하기 > 예약/신청)를 상단에 상시 노출하여 정보 탐색 중 길을 잃지 않도록 가이드함

---

### ♿ 웹 접근성 표준 준수 (Accessibility)
